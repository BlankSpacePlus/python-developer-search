...
def index():
    ...
    print('ip:', ip, 't:', t)
    with open("../log/api_search.log", "a", encoding="UTF-8") as f:
        # 追加写模式
        f.write(f"{ip} {t}\n")
...
def search(query):
    ...
    print(query)
    results, result_count = get_api_top_k(query)
    end_time = time.time()
...
def init_index():
    ...
    schema = Schema(methodName=STORED(), methodContent=TEXT(stored=True, analyzer=StemmingAnalyzer()), methodNotes=STORED())
    files = ["api_json"]
    api_list = list()
    for file_name in files:
        with open(f"../data/{file_name}.json", 'r', encoding="UTF-8") as f:
            data = json.load(f)
        api_list.extend(data)
    index_dir = '../data/api_index/'
    ...
    if exists_in(index_dir):
        ...
        for s in api_list:
            ...
            methodContent = s["methodContent"]
            methodNotes = s["methodNotes"]
            writer.add_document(methodName=methodName, methodContent=methodContent, methodNotes=methodNotes)
        writer.commit()
        print("end of building index ...")
    field_name = "methodContent"  # 默认的field
...
def get_api_top_k(query: str, k: int = 10):
    init_index()
    results = list()
    global INDEX, PARSER
    ...
    with INDEX.searcher() as s:
        ...
        if results_count != 0:
            new_results = []
            for _ in results[:k]:  # 取前5个结果
                print(_)
                new_results.append({"methodName": _["methodName"], "methodContent": _["methodContent"], "methodNotes": _["methodNotes"]})
            #results = [_ for _ in results[:k]]  # 取前5个结果
        print(new_results)
    return new_results, results_count
