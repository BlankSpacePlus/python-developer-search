...
def query_parse(query, path_parsed_vocab, path_method_vocab):
    ...
    """
    vjdk = dict(code_matcher_utils.load_pkl(path_parsed_vocab))
    vword = dict(code_matcher_utils.load_pkl(path_method_vocab))
    ...
    tokens = code_matcher_utils.get_tokens(query)
    ...
    for token in tokens:
        ...
        if ttype in type_all:
            ...
            if stem in vword:
                ...
                freq = []
                syns = code_matcher_utils.get_synonyms(stem)  #理解为获取stem的同义词
                for syn in syns:
                    score = 0
                    stem = code_matcher_utils.get_stemmed(syn)
                    if stem in vword:
                        score = vword[stem]
                    freq.append(score)
                idx_max_freq = -1
            ...
            if ttype in type_nn and stem in vjdk:
                para = 2
                impact = vjdk[stem]
            tvalue = code_matcher_utils.get_stemmed(tvalue)
